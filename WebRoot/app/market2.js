...
function search_database() {
    ...
    if(!params["catagoryId"]) params["catagoryId"] = "2";
    table1.datagrid("load", { "params": JSON.stringify(params)}); 
};
...
$.getJSON("../resources/data/database.json",function (rs) {
    $("#tree1").tree("loadData", rs[1].children);
    $("#tree2").combotree("loadData", rs[1].children);
});
...
var toolbar = [{
    ...
    handler: function() {
        flag = "add";
        dlg1.dialog("open").dialog('setTitle','添加市场分析');
        form1.form("clear");
        editor.setData("");
    }
},
'-', {
    ...
    handler: function() {
        ...
        if (r) {
            form1.form("load", r); 
            dlg1.dialog("open").dialog('setTitle','修改市场分析');
...
dfd_datagrid.done(function () {
    table1.datagrid({
        ...
        queryParams:{ params: JSON.stringify({catagoryId: "2"}) },
        pagination: true, 
        rownumbers:true, 
        fitColumns:true, 
        singleSelect:true, 
