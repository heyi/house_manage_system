...
function search_database() {
    ...
    if(!params["catagoryId"]) params["catagoryId"] = "1";
    table1.datagrid("load", { "params": JSON.stringify(params)}); 
};
...
$.getJSON("../resources/data/database.json",function (rs) {
    ...
    $("#tree2").combotree("loadData", rs[0].children);
});
...
var toolbar = [{
    ...
    handler: function() {
        flag = "add";
        dlg1.dialog("open").dialog('setTitle','添加政策法规');
        form1.form("clear");
        editor.setData("");
    }
},
'-', {
    ...
    handler: function() {
        ...
        if (r) {
            form1.form("load", r); 
            dlg1.dialog("open").dialog('setTitle','修改政策法规');
...
dfd_datagrid.done(function () {
    table1.datagrid({
        ...
        queryParams:{ params: JSON.stringify({catagoryId: "1"}) },
        pagination: true, 
        rownumbers:true, 
        fitColumns:true, 
        singleSelect:true, 
